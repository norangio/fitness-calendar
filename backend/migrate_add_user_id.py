...
default_user = os.environ.get("MIGRATION_DEFAULT_USER", "local-user")
default_user_sql = default_user.replace("'", "''")
...
if "user_id" not in columns:
    ...
    cursor.execute(f"ALTER TABLE activities ADD COLUMN user_id TEXT NOT NULL DEFAULT '{default_user_sql}'")
    cursor.execute("CREATE INDEX IF NOT EXISTS ix_activities_user_id ON activities(user_id)")
    cursor.execute("SELECT COUNT(*) FROM activities")
    count = cursor.fetchone()[0]
    print(f"  Backfilled {count} rows with user_id='{default_user}'")
else:
...
if "user_id" not in columns:
    ...
    cursor.execute(f"ALTER TABLE body_logs ADD COLUMN user_id TEXT NOT NULL DEFAULT '{default_user_sql}'")
