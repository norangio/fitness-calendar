    ...
    <div className="flex flex-col flex-1 min-h-0">
      <div className="grid grid-cols-7 border-b border-stone-200 dark:border-stone-700">
        {WEEKDAYS_LONG.map((d, i) => (
          <div key={d} className="px-1 sm:px-2 py-1 sm:py-2 text-xs font-medium text-stone-500 text-center dark:text-stone-400">
      ...
      <div className="grid grid-cols-7 flex-1 auto-rows-fr min-h-0">
        {days.map((day) => {
          ...
          return (
            <div
              key={key}
              className={`border-b border-r border-stone-200 p-0.5 sm:p-1.5 overflow-hidden cursor-pointer hover:bg-stone-100 transition-colors dark:border-stone-700/50 dark:hover:bg-stone-700/30 ${
                !inMonth ? 'opacity-30' : ''
              }`}
            ...
            >
              <div className="flex items-center gap-1 mb-1">
                <div className={`text-xs font-medium w-6 h-6 flex items-center justify-center rounded-full ${
                  today ? 'bg-stone-900 text-white dark:bg-stone-100 dark:text-stone-900' : 'text-stone-500 dark:text-stone-400'
                }`}>
                  {format(day, 'd')}
                </div>
              ...
              <div className="space-y-0.5">
                ...
                {dayActivities.length > 3 && (
                  <div className="text-[10px] text-stone-400 pl-2 dark:text-stone-500">+{dayActivities.length - 3} more</div>
                )}
              </div>
            </div>
          );
