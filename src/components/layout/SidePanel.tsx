...
export function SidePanel({ activities, bodyLogs = [], viewMode, dateRange }: SidePanelProps) {
  ...
  return (
    <aside className="flex flex-col gap-4 border-l border-stone-200 bg-stone-50/80 p-5 overflow-y-auto w-80 shrink-0 dark:border-stone-700 dark:bg-stone-800/50">
      <div>
        <h2 className="text-sm font-medium text-stone-500 uppercase tracking-wider dark:text-stone-400">Summary</h2>
        <div className="mt-3 grid grid-cols-2 gap-3">
          <div className="rounded-lg bg-stone-200/60 p-3 dark:bg-stone-700/50">
            <div className="text-2xl font-bold text-stone-900 dark:text-stone-100">{activities.length}</div>
            <div className="text-xs text-stone-500 dark:text-stone-400">Activities</div>
          </div>
          <div className="rounded-lg bg-stone-200/60 p-3 dark:bg-stone-700/50">
            <div className="text-2xl font-bold text-stone-900 dark:text-stone-100">{totalHours}h</div>
            <div className="text-xs text-stone-500 dark:text-stone-400">Total Time</div>
          </div>
        </div>
      ...
      <div>
        <h2 className="text-sm font-medium text-stone-500 uppercase tracking-wider mb-3 dark:text-stone-400">By Type</h2>
          ...
          {uniqueTypes.map((type) => {
            ...
            return (
              <div key={type} className="flex items-center justify-between text-sm">
                ...
                <span className="text-stone-500 dark:text-stone-400">{count} · {Math.round(mins / 6) / 10}h</span>
              </div>
            );
          })}
        </div>
      ...
      <div>
        <h2 className="text-sm font-medium text-stone-500 uppercase tracking-wider mb-3 dark:text-stone-400">Activity Hours</h2>
